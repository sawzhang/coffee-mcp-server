"""Coffee Company MCP CLI — interactive client for the Coffee Company MCP Server."""
...
async def _interactive():
    console.print(Panel.fit(
        "[bold green]Coffee Company MCP CLI[/bold green] (B2B Open Platform)\n"
        "基于真实 openapi.coffeecompany.com 接口映射\n"
    ...
    async with (await _create_session()) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            commands = {
                "member": ("member_query", "查询会员 - member <手机号/openId/memberId>"),
                "tier": ("member_tier", "会员等级 - tier <memberId>"),
                "benefits": ("member_benefits", "权益状态 - benefits <memberId>"),
                "coupons": ("member_benefit_list", "券列表 - coupons <memberId>"),
                ...
                "assets": ("assets_list", "客户资产 - assets <memberId>"),
                "pay": ("cashier_pay_query", "支付查询 - pay <payToken>"),
            }

            while True:
                try:
                    user_input = console.input("[bold cyan]coffee>[/bold cyan] ").strip()
                ...
                if user_input == "data":
                    console.print(Panel(
                        "[bold]Demo 测试数据[/bold]\n\n"
                        "[cyan]会员:[/cyan]\n"
                        "  CC_M_100001 (张三, 金星, 138****1234)\n"
                        "  CC_M_100002 (李四, 银星, 139****5678)\n"
                        "  CC_M_100003 (王五, 钻星, 137****9012)\n\n"
                        "[cyan]券码:[/cyan]\n"
                        "  CC20260301A001 (未使用, 中杯饮品券)\n"
                        "  CC20260301A002 (已使用)\n"
                        "  CC20260215B001 (买一送一券)\n\n"
                        "[cyan]订单号:[/cyan]\n"
                ...
                try:
                    if cmd == "member":
                        if not args:
                            console.print("[red]请提供手机号/openId/memberId[/red]")
                            continue
                        v = args[0]
                        if v.startswith("CC_"):
                            arguments["member_id"] = v
                        elif v.startswith("o"):
                            arguments["open_id"] = v
                        else:
                            arguments["mobile"] = v
                        ...
                        if not args:
                            console.print(f"[red]请提供 memberId，如: {cmd} CC_M_100001[/red]")
                            continue
                        arguments["member_id"] = args[0]
                        ...
                        if not args:
                            console.print("[red]请提供券码，如: coupon CC20260301A001[/red]")
                            continue
...
async def _demo():
    console.print(Panel.fit(
        "[bold green]Coffee Company MCP Server Demo[/bold green]\n"
    ...
    async with (await _create_session()) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            steps = [
                ...
                ("member_benefit_list", {"member_id": "CC_M_100001"},
...
@click.group()
def cli():
    """Coffee Company MCP CLI — Coffee Company B2B 开放平台 MCP 客户端"""
    pass
...
def member(identifier):
    """查询会员信息（手机号/openId/memberId）"""
    if identifier.startswith("CC_"):
        args = {"member_id": identifier}
...
def tier(member_id):
    """查询会员等级"""
    asyncio.run(_run_single("member_tier", {"member_id": member_id}))
...
def benefits(member_id):
    """查询会员权益状态"""
    asyncio.run(_run_single("member_benefits", {"member_id": member_id}))


@cli.command()
@click.argument("member_id")
def assets(member_id):
    """查询客户资产"""
    asyncio.run(_run_single("assets_list", {"member_id": member_id}))
